...
2. **Run all commands yourself.** Do not ask the user to run shell commands — use the exec tool to run them directly. This includes: spawn worktrees, lock claims, installs, builds, tests, dev servers, git operations, etc. Only ask the human for: (a) product decisions, (b) credentials/secrets, or (c) explicit permission gates like "Do you want me to commit/push/open a PR?"
