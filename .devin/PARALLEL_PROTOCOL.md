...
2. **Verify your working directory.** Run `pwd`. It MUST be a path under `../worktrees/<slug>/` matching your slug. Working in the main checkout (the directory containing `.git/` as a real directory rather than as a `gitdir:` pointer) is FORBIDDEN, even when no other instance is running — `git switch` there will clobber any sibling worktree's checkout.
   - If you are in the main checkout: **STOP and fix it immediately by running** `bash scripts/devin-spawn.sh <slug>` **yourself**, then `cd` into the printed worktree path, and only then continue. Do not ask the human to run commands for you.
...
- **Command ownership:** if anything needs to be run (spawn worktrees, lock claims, installs, builds, tests, dev servers, git operations), the AI agent runs it. Only ask the human for (a) product decisions, (b) credentials/secrets, or (c) explicit permission gates ("Do you want me to commit/push/open a PR?").
- **Backend refresh rule (tell the human when to repull/restart):** if you changed anything under `apps/server/`, `packages/shared/` (server runtime depends on emitted `dist/`), `pnpm-lock.yaml`/workspace deps, or `apps/server/src/db/migrations/`, you MUST explicitly tell the human that any already-running backend process needs to be restarted to pick up the changes.
  - If the human runs the backend from a different checkout/worktree, tell them to `git pull` (or re-run their worktree update flow) before restarting.
  - Also tell them to restart when you see symptoms like: API route mismatch (404), schema/migration errors on boot, or the web UI behaving as if old API/types are still in effect.
