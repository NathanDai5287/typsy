...
- `src/components/` — `Nav.tsx` (top nav, hidden during onboarding; reflects the active focus layer with a yellow accent + active-tab cursor), `StatusBar.tsx` (vim-style bottom bar with route + layer-aware keymap hints), `HelpOverlay.tsx` (full-screen `?` shortcut reference), `KeyboardVisual.tsx` (on-screen keyboard with finger colors + opacity-fade for muscle memory + heatmap support), and `FingeringEditor.tsx` (reusable click-or-keyboard editor used in onboarding and on `/fingering`).
...
- `src/lib/keymapContext.tsx` — `KeymapProvider` (wraps the whole app, owns the global keymap + the help overlay state + the Monkeytype-style focus layer (`'content' | 'navbar'`); also runs an Esc → enter-navbar handler at the **window** level + a capture-phase navbar handler that walks tabs on `←/→` and exits on `Enter`/`Esc`) and `useRegisterPageKeymap(title, bindings)` (pages call this to register their own bindings AND have them appear in the help overlay). When entering the navbar layer it prefetches the cross-tab queries (`layouts/summary`, `sessions[layoutId]`, `ngramStats[layoutId]`) so arrow-key tab switching reuses cache.
...
- **Practice page ↔ keymap interaction is delicate.** The typing handler runs at **capture phase** so it sees keystrokes before any bubble-phase keymap listener. When it consumes a typed character it calls `stopImmediatePropagation`, which prevents anything else from re-processing the same key. Modified keys (`?` = `Shift+Slash`, etc.) bypass typing entirely — the typing handler returns early when any modifier is held, letting the bubble-phase global keymap take over. The handler also bails out when the navbar focus layer is active so letters in nav mode aren't typed. **Don't add bare-letter shortcuts to the global keymap** — they'd be eaten by typing on the practice page. Use modified keys (`Shift+X`), or non-alpha keys like `?`, `Esc`, `Tab`, `\`.
- **Navigation uses a Monkeytype-style two-layer focus model, not a `g`-leader.** The `KeymapProvider` owns `layer: 'content' | 'navbar'`. Pressing **Esc** lifts focus to the navbar layer (unless a page binds Esc itself — Practice's "end session" handler still wins because it stops propagation on `document` before the event reaches the global handler on `window`); on Practice, the Esc handler co-fires `enterNavbarLayer` so the gesture still ends in the navbar. In navbar layer a capture-phase listener handles `←/→` (walk `NAV_TABS`), `Enter` (return to content), `Esc` (cancel), and swallows everything else; clicking outside the nav also drops the layer. `Nav.tsx` reflects the layer with a yellow top accent + active-tab cursor; `StatusBar.tsx` shows `[NAV]` / `[CONTENT]` and layer-aware key hints. There is no `g`-leader and no `Shift+<letter>` route shortcut anymore.
