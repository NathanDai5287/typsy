...
| Layout grid definitions (QWERTY/Colemak/Graphite) + builders | `packages/shared/src/layouts.ts` |
...
- **CI auto-merges on human review approval.** `.github/workflows/ci.yml` runs build+test on every PR push. When CI is green AND the PR has at least one current APPROVED review (the standard GitHub Approve button — Files → Review changes → Approve), the workflow enables auto-merge (merge commit) and deletes the branch on merge. There is no merge-trigger label; gating uses GitHub's native review system. PRs are opened as regular (non-draft) PRs.
